...
# remove num_comments VIF=6.372740
...
model_path = paste("Python/", dataset, "/basic_close", ".RData", sep="")
...
model_basic_close <- lmer(
    formula = formula,
    verbose=TRUE,
    data=selected_df
)
car::vif(model_basic_close)

# calculate R-square
library(MuMIn)
as.data.frame(r.squaredGLMM(model_basic_close))

save(model_basic_close, file=model_path)
print("finish build up basic close model")
