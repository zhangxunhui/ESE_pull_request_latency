...
keep_factors = c('followers', 'prev_pullreqs', 'integrator_availability', 'open_pr_num', 'project_age', 'team_size', 'description_length', 'commits_on_files_touched_open', 'files_changed_open', 'num_commits_open', 'src_churn_open', 'test_churn_open', 'core_member', 'ci_exists', 'friday_effect', 'test_inclusion_open', 'contrib_gender', 'asserts_per_kloc', 'sloc')
...
model_path = paste("Python/", dataset, "/basic_open", ".RData", sep="")
...
model_basic_open <- lmer(
    formula = formula,
    verbose=TRUE,
    data=selected_df
)
car::vif(model_basic_open)

# calculate R-square
library(MuMIn)
as.data.frame(r.squaredGLMM(model_basic_open))

save(model_basic_open, file=model_path)
print("finish build up basic open model")
