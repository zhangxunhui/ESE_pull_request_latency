# build for pull request using CI tools and not using CI tools
...
keep_factors = c('followers', 'prev_pullreqs', 'integrator_availability', 'open_pr_num', 'project_age', 'team_size', 'description_length', 'num_code_comments', 'commits_on_files_touched_close', 'files_changed_close', 'num_commits_close', 'src_churn_close', 'test_churn_close', 'core_member', 'friday_effect', 'hash_tag', 'test_inclusion_close', 'same_user', 'has_comments', 'contrib_gender', 'prior_review_num', 'sloc', 'test_lines_per_kloc', 'reopen_or_not')

categorical_factors = c("core_member", "friday_effect", "test_inclusion_close", "contrib_gender", "hash_tag", "same_user", "has_comments", "reopen_or_not")
binary_factors = c("core_member", "friday_effect", "test_inclusion_close", "contrib_gender", "hash_tag", "same_user", "has_comments", "reopen_or_not")
...
selected_df = df[c(keep_factors, "lifetime_minutes", "project_id", "ci_exists")]
...
train_model_func <- function(selected_df, which_model) {
    ...
    model_context_ci_exists <- lmer(
        formula = formula,
        verbose=TRUE,
        data=selected_df
    )
    vifs = car::vif(model_context_ci_exists)
    print(vifs)

    # calculate R-square
    library(MuMIn)
    Rs = as.data.frame(r.squaredGLMM(model_context_ci_exists))
    print(Rs)

    save(model_context_ci_exists, file=model_path)
    print(paste("finish build up model for: ", which_model, sep=""))
}


# 1. use CI
use_ci_df = selected_df[selected_df$ci_exists==1,]

# 2. no CI
no_ci_df = selected_df[selected_df$ci_exists==0,]


train_model_func(use_ci_df, "context_use_ci")
train_model_func(no_ci_df, "context_no_ci")
