...
train_model_func <- function(selected_df, which_model) {
    ...
    model_context_workload <- lmer(
        formula = formula,
        verbose=TRUE,
        data=selected_df
    )
    vifs = car::vif(model_context_workload)
    print(vifs)

    # calculate R-square
    library(MuMIn)
    Rs = as.data.frame(r.squaredGLMM(model_context_workload))
    print(Rs)

    save(model_context_workload, file=model_path)
    print(paste("finish build up model for: ", which_model, sep=""))
}


# 1. small workload (<=6)
small_workload_df = selected_df[selected_df$open_pr_num<=6,]

# 2. medium workload (>6, <=29)
medium_workload_df = selected_df[(selected_df$open_pr_num>6) & (selected_df$open_pr_num<=29),]

# 3. large workload (>29)
large_workload_df = selected_df[selected_df$open_pr_num>29,]


train_model_func(small_workload_df, "context_workload_small")
train_model_func(medium_workload_df, "context_workload_medium")
train_model_func(large_workload_df, "context_workload_large")
