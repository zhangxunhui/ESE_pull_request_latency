...
# remove num_comments, vif=6.594979
keep_factors = c('followers', 'prev_pullreqs', 'integrator_availability', 'open_pr_num', 'project_age', 'team_size', 'ci_latency', 'description_length', 'num_code_comments', 'commits_on_files_touched_close', 'files_changed_close', 'num_commits_close', 'src_churn_close', 'test_churn_close', 'core_member', 'ci_test_passed', 'friday_effect', 'hash_tag', 'test_inclusion_close', 'same_user', 'has_comments', 'contrib_gender', 'prior_review_num', 'sloc', 'test_lines_per_kloc', 'reopen_or_not')

categorical_factors = c("core_member", "has_comments", "friday_effect", "test_inclusion_close", "contrib_gender", "hash_tag", "same_user", 'reopen_or_not', 'ci_test_passed')
binary_factors = c("core_member", "has_comments", "friday_effect", "test_inclusion_close", "contrib_gender", "hash_tag", "same_user", 'reopen_or_not', 'ci_test_passed')
...
q = "select * from new_pullreq_ese where ci_exists=1"
...
model_path = paste("Python/", dataset, "/special_ci_exists", ".RData", sep="")
...
selected_df$ci_test_passed = relevel(selected_df$ci_test_passed, ref = '0')
model_special_ci_exists <- lmer(
    formula = formula,
    verbose=TRUE,
    data=selected_df
)
car::vif(model_special_ci_exists)

# calculate R-square
library(MuMIn)
as.data.frame(r.squaredGLMM(model_special_ci_exists))

save(model_special_ci_exists, file=model_path)
print("finish build up special ci exists model")
