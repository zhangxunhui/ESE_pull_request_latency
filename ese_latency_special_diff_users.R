...
# remove num_comments, vif:5.963736
keep_factors = c('followers', 'prev_pullreqs', 'integrator_availability', 'open_pr_num', 'project_age', 'team_size', 'description_length', 'num_code_comments', 'commits_on_files_touched_close', 'files_changed_close', 'num_commits_close', 'src_churn_close', 'test_churn_close', 'core_member', 'ci_exists', 'friday_effect', 'hash_tag', 'test_inclusion_close', 'has_comments', 'contrib_gender', 'prior_review_num', 'asserts_per_kloc', 'sloc', 'comment_conflict', 'reopen_or_not')
...
q = "select * from new_pullreq_ese where same_user=0"
...
model_path = paste("Python/", dataset, "/special_diff_users", ".RData", sep="")
...
model_special_diff_users <- lmer(
    formula = formula,
    verbose=TRUE,
    data=selected_df
)
car::vif(model_special_diff_users)

# calculate R-square
library(MuMIn)
as.data.frame(r.squaredGLMM(model_special_diff_users))

save(model_special_diff_users, file=model_path)
print("finish build up special diff users model")
