...
keep_factors = c('first_response_time', 'followers', 'prev_pullreqs', 'integrator_availability', 'open_pr_num', 'project_age', 'team_size', 'description_length', 'num_code_comments', 'num_comments', 'commits_on_files_touched_close', 'files_changed_close', 'num_commits_close', 'src_churn_close', 'test_churn_close', 'core_member', 'ci_exists', 'friday_effect', 'hash_tag', 'test_inclusion_close', 'same_user', 'contrib_gender', 'prior_review_num', 'sloc', 'test_lines_per_kloc', 'reopen_or_not')

categorical_factors = c("core_member", "ci_exists", "friday_effect", "test_inclusion_close", "contrib_gender", "hash_tag", "same_user", 'reopen_or_not')
binary_factors = c("core_member", "ci_exists", "friday_effect", "test_inclusion_close", "contrib_gender", "hash_tag", "same_user", 'reopen_or_not')
...
q = "select * from new_pullreq_ese where has_comments=1"
...
model_path = paste("Python/", dataset, "/special_has_comments", ".RData", sep="")
...
model_special_has_comments <- lmer(
    formula = formula,
    verbose=TRUE,
    data=selected_df
)
car::vif(model_special_has_comments)

# calculate R-square
library(MuMIn)
as.data.frame(r.squaredGLMM(model_special_has_comments))

save(model_special_has_comments, file=model_path)
print("finish build up special has comments model")
